...
NK_API struct nk_context*   nk_glfw3_init(uint32_t w, uint32_t h);
NK_API void					nk_glfw3_update(uint32_t w, uint32_t h);
...
nk_glfw3_init(uint32_t w, uint32_t h)
{
...
NK_API void 
nk_glfw3_update(uint32_t w, uint32_t h)
{
	glfw.width = w;
	glfw.height = h;
	glfw.display_width = w;
