...
VPATH := ./source:./source/Math:./source/Timer

# libs finding and binding
iUNAME_S := $(shell uname -s)
GLFW3 := $(shell pkg-config --libs glfw3)
ifeq ($(UNAME_S),Darwin)
	LIBS := $(GLFW3) -framework OpenGL -framework Cocoa -framework IOKit -framework CoreVideo -lm -lGLEW -L/usr/local/lib -L./libraries/FMOD/lib/macOS -llibfmod -llibfmodL
	CPPFLAGS += -I/usr/local/include
else
	LIBS := $(GLFW3) -lGL -lm -lGLU -lGLEW -llibfmod -llibfmodL
endif
